//
//  UpdateCheckerApp.swift
//  UpdateChecker
//
//  Created by Constantin Clerc on 13/12/2022.
//

import SwiftUI

@main
struct UpdateCheckerApp: App {
    var body: some Scene {
        WindowGroup {
            ContentView()
            //Keep from here if you don't have a onAppear
                .onAppear {
                    //Keep start
                    //Please note that you can change the Github repo by yours.
                    if let version = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String, let url = URL(string: "https://api.github.com/repos/c22dev/UpdateChecker/releases/latest") {
                            ...
                            if let json = try? JSONSerialization.jsonObject(with: data, options: .mutableContainers) as? [String: Any] {
                                if json["tag_name"] as? String != version {
                                    ...
                                    UIApplication.shared.confirmAlert(title: "Update available", body: "Do you want to download the update from the Github ?", onOK: {
                                        UIApplication.shared.open(URL(string: "https://github.com/c22dev/UpdateChecker/releases/latest")!)
                                    }, noCancel: false)
            ...
            //Keep end for people with .onAppear {
        }
    }
}
